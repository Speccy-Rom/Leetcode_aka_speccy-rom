...
func maxWalls(robots []int, distance []int, walls []int) int {
	...
	dfs = func(i, j int) int {
		...
		if i+1 < n {
			if j == 0 {
				right = minInt(right, arr[i+1].x-arr[i+1].d-1)
			} else {
				right = minInt(right, arr[i+1].x-1)
			}
		}
		l = sort.SearchInts(walls, arr[i].x)
		r = sort.SearchInts(walls, right+1)
		ans = maxInt(ans, dfs(i-1, 1)+(r-l))
...
func main() {
	maxWalls([]int{3, 8}, []int{4, 2}, []int{1, 2, 4, 7, 9})
}

...
func predictPartyVictory(senate string) string {
	radiant := make([]int, 0) // to store the indices of Radiant senators
	dire := make([]int, 0)    // to store the indices of Dire senators
	...
	for len(radiant) > 0 && len(dire) > 0 {
		...
		// remove the banned senator from the queue
		radiant = radiant[1:]
		dire = dire[1:]
	}
